...
const SellUserInfo = () => {
  ...
  return (
    <div tw="flex flex-col sm:flex-row items-end sm:justify-between sm:items-center">
      <div tw="flex items-center">
        <img alt="metamask" src={user?.avatar} tw="w-28 sm:w-32 rounded-full border-4 border-white"/>
        <div tw="sm:mb-6">
          <div tw="flex items-center px-6">
            <div tw="text-base sm:text-2xl whitespace-nowrap max-w-sm font-semibold text-gray-100">{user?.name}</div>
            <div tw="text-white text-xs whitespace-nowrap bg-violet-200 py-0.5 px-2 ml-3 mt-1 rounded-full" style={{height: 'fit-content'}}>LEVEL 8</div>
          </div>
          <div tw="flex flex-col sm:flex-row justify-center sm:items-center px-6 py-0.5"> 
            <div tw='flex items-center'>
            ...
            <div tw="text-gray-600 text-xs sm:ml-3">Joined {joinDateFormat(user?.created)}</div>
          </div>
        </div>
      </div>
      <div tw="flex justify-center items-center mt-2.5 sm:mb-6">
